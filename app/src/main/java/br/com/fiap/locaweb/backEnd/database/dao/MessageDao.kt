...
interface MessageDao {
    ...
    @Query("SELECT mensagem.*, contato.nome as senderName, contato.email as senderEmail FROM mensagem INNER JOIN contato ON mensagem.remetente = contato.email WHERE destinatario = :recipientEmail AND importante = 0 ORDER BY mensagem.created_at")
    fun getMessagesWithSenderInfoForRecipient(recipientEmail: String): List<Message>

    @Query("SELECT * FROM mensagem WHERE importante = 1 AND destinatario = :recipientEmail ORDER BY created_at")
